...
class SemeProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        return [
            self.example(i, l)
            for i, l in enumerate(self._read_data(fn, quotechar='"'))
        ]
    ...
    def get_test_examples(self, data_dir):
        fn = os.path.join(data_dir, 'SemEval2016-Task6-subtaskA-testdata-gold.txt')
        return [
            self.example(i, l)
            for i, l in enumerate(self._read_data(fn, delimiter='\t'))
        ]
...
def get_estimator(
    ...
    return estimator, train_examples, num_train_steps

def get_train_input_fn(processor, tokenizer, train_examples):
    train_file = os.path.join(FLAGS.output_dir, "train.tf_record")
    file_based_convert_examples_to_features(
        train_examples, processor.get_labels(), FLAGS.max_seq_length,
        tokenizer, train_file)

    return file_based_input_fn_builder(
        input_file=train_file,
        seq_length=FLAGS.max_seq_length,
        is_training=True,
        drop_remainder=True)

def get_eval_input_fn(processor, tokenizer):
    eval_examples = processor.get_dev_examples(FLAGS.data_dir)
    num_actual_eval_examples = len(eval_examples)

    eval_file = os.path.join(FLAGS.output_dir, "eval.tf_record")
    file_based_convert_examples_to_features(
        eval_examples, processor.get_labels(), FLAGS.max_seq_length, tokenizer, eval_file)
...
def predict(processor, tokenizer, estimator):
    ...
    file_based_convert_examples_to_features(
        predict_examples, processor.get_labels(), FLAGS.max_seq_length,
...
def main(_):
    ...
    eval_input_fn = get_eval_input_fn(processor, tokenizer)
